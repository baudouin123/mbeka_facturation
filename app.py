...
app.config['RESET_MAIL_PASSWORD'] = 'YannickSimba123@'  # ⚠️ À REMPLIR
...
def generer_pdf_facture(data, chemin_pdf, type_facture='client'):
    ...
    if type_facture == 'client':
        ...
        c.drawRightString(COL_PRIX + 1.2*cm, y - 0.55*cm, "PRIX HT")
        c.drawRightString(COL_TOTAL + 1.5*cm, y - 0.55*cm, "TOTAL")
    else:
        c.drawString(COL_DESC, y - 0.55*cm, "DESCRIPTION")
    ...
    if type_facture == 'client':
        if appliquer_tva:
            ...
            c.drawRightString(width - 1.5*cm, y - 2.2*cm, f"{total_ttc:.2f} €")
        else:
            # Sans TVA
            c.setFillColor(GRIS_CLAIR)
    ...
    if data.get('notes'):
        ...
        max_chars = 95
        lignes = []
        while len(notes_text) > max_chars:
            pos = notes_text[:max_chars].rfind(' ')
            if pos == -1:
                pos = max_chars
            lignes.append(notes_text[:pos])
...
def generer_facture_client():
    ...
    try:
        ...
        facture = Facture(
            ...
            type_facture='client',
            date_facture=datetime.strptime(pdf_data['date_facture'], "%d/%m/%Y").date(),
...
class Role(db.Model):
    ...
    def to_dict(self):
        nb_users = Utilisateur.query.filter_by(role=self.code).count()
        return {
            'id': self.id,
            'nom': self.nom,
            'code': self.code,
...
# ========================================================================
# FONCTION D'INITIALISATION DES RÔLES SYSTÈME
# ========================================================================

def initialiser_roles_systeme():
    """Crée les 3 rôles système par défaut s'ils n'existent pas"""
...
def api_creer_role():
    ...
    try:
        ...
        code = data['nom'].lower().replace(' ', '_').replace('é', 'e').replace('è', 'e').replace('à', 'a')
        ...
        creer_log('ROLE_CREATION', f'Rôle "{nouveau_role.nom}" créé', current_user)
    ...
    try:
        ...
        if role.est_systeme:
            if 'permissions' in data:
                ...
                db.session.commit()
                creer_log('ROLE_MODIFICATION', f'Permissions du rôle "{role.nom}" modifiées', current_user)
        ...
        creer_log('ROLE_MODIFICATION', f'Rôle "{role.nom}" modifié', current_user)
...
def api_supprimer_role(role_id):
    ...
    try:
        ...
        creer_log('ROLE_SUPPRESSION', f'Rôle "{nom}" supprimé', current_user)
...
def api_dupliquer_role(role_id):
    ...
    try:
        ...
        creer_log('ROLE_DUPLICATION', f'Rôle "{role_original.nom}" dupliqué', current_user)
